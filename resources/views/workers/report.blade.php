<x-layouts.app>
    ...
    @if (!is_null($latestFile))
        ...
        @else
            ...
            <button id="getLatestReport"
                class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-full text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700 ml-3 btnDownloadFile"
                data-file="{{ $latestFile['full_path'] }}">Baixar último relatório</button>
        @endif
    @endif

    <form action="{{ route('workers.make-report') }}" method="POST">
        @csrf
        <button id="makeNewReport"
            class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 "
            type="submit">Gerar Novo relatório</button>
    </form>
    ...
    @if (!empty($files))
        ...
        <h2 class="text-2xl font-extrabold mb-3">Relatórios solicitados</h2>
